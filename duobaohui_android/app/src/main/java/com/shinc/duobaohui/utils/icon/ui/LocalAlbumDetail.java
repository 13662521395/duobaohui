    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                //获取该文件夹下地所有文件
                final List<LocalImageHelper.LocalFile> folders = LocalImageHelper.getInstance().getFolder(folder);
                    ...
                    public void run() {
                        ...
                        if (folders != null) {
                            ...
                            if (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize() > 0) {
                                finish.setText("完成(" + (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize()) + "/" + MyApplication.ImageViewNum + ")");
                                finish.setEnabled(true);
                                headerFinish.setText("完成(" + (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize()) + "/" + MyApplication.ImageViewNum + ")");
...
//                                headerFinish.setEnabled(false);
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        if (!b) {
            ...
            if (!checkedItems.contains(compoundButton.getTag())) {
                if (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize() >= MyApplication.ImageViewNum) {
