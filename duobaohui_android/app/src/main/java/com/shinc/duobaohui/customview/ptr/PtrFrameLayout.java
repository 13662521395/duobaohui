...
import com.shinc.duobaohui.customview.ptr.indicator.PtrIndicator;
import com.shinc.duobaohui.customview.ptr.util.PtrCLog;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public final static byte PTR_STATUS_PREPARE = 2;//标示，下拉prepare;
    public final static byte PTR_STATUS_LOADING = 3;//正在加载loading;
    public final static byte PTR_STATUS_COMPLETE = 4;// 加载结束；
    ...
    protected View mContent;//View ，用于承载控件；
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPtrIndicator = new PtrIndicator();//创建PtrIndicator;

        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.PtrFrameLayout, 0, 0);//读取自定义属性；
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.PtrFrameLayout_ptr_header, mHeaderId);//头部的Id；
            mContainerId = arr.getResourceId(R.styleable.PtrFrameLayout_ptr_content, mContainerId);//容器的Id；

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));//设置滑动的阻尼系数；

            mDurationToClose = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close, mDurationToClose);//回弹延时，默认 200ms，回弹到刷新高度所用时间。
            mDurationToCloseHeader = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);//头部回弹时间，默认 1000ms。

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();//读取PtrIndicator中的Ratio的默认值(比例);
            ratio = arr.getFloat(R.styleable.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);//读取Ratio的属性；
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);//设置Ratio属性；

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.PtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);//读取在刷新时是否控制头部保持展开的效果；

            mPullToRefresh = arr.getBoolean(R.styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);//下拉刷新 / 释放刷新，默认为释放刷新。
            arr.recycle();
        }

        mScrollChecker = new ScrollChecker();//创建ScrollChecker对象；

        final ViewConfiguration conf = ViewConfiguration.get(getContext());//获得一些关于timeouts(时间)、sizes(大小)、distances(距离)的标准常量值 。
        mPagingTouchSlop = conf.getScaledTouchSlop() * 2;//获得一个触摸移动的最小像素值。也就是说，只有超过了这个值，才代表我们该滑屏处理了。
    }


    /**
     * 获取mPagindTouchSlop的值;
     * @return
     */
    public int getmPagingTouchSlop() {
        return mPagingTouchSlop;
    }

    /**
     * 顾名思义：
     * 当View中所有的子控件均从xml中解析映射成对象后触发；
     */
    @Override
    protected void onFinishInflate() {
        final int childCount = getChildCount();//拿到子View的数量；
        if (childCount > 2) {   //数量不能大于两个；
            throw new IllegalStateException("PtrFrameLayout only can host 2 elements");
        } else if (childCount == 2) {
            if (mHeaderId != 0 && mHeaderView == null) {
                mHeaderView = findViewById(mHeaderId);//找header;
            }
            if (mContainerId != 0 && mContent == null) {
                mContent = findViewById(mContainerId);//找Container;
            }
            ...
            if (mContent == null || mHeaderView == null) {
                ...
                if (child1 instanceof PtrUIHandler) {           //通过instanceof区分头和容器；
                    mHeaderView = child1;
                    mContent = child2;
            ...
            mContent = getChildAt(0);//如果只有一个子view的时候，
        } else {    //当子View的个数小于1的时候，认为出现错误，通过在容器中创建一个TextView 来承载错误提示。
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            measureChildWithMargins(mHeaderView, widthMeasureSpec, 0, heightMeasureSpec, 0);//某一个子view，多宽，多高, 内部加上了viewGroup的padding值、margin值和传入的宽高wUsed、hUsed  ;
            MarginLayoutParams lp = (MarginLayoutParams) mHeaderView.getLayoutParams();//获得headerView的LayoutParams；
            mHeaderHeight = mHeaderView.getMeasuredHeight() + lp.topMargin + lp.bottomMargin;//将header实际的高度加上头margin+底margins;得到实际的高度；
            mPtrIndicator.setHeaderHeight(mHeaderHeight);//向PtrIndicator中设置Header的实际高度；
        }

        if (mContent != null) {
            measureContentView(mContent, widthMeasureSpec, heightMeasureSpec);//测量容器view;
            if (DEBUG && DEBUG_LAYOUT) {
    ...
    private void layoutChildren() {
        int offsetX = mPtrIndicator.getCurrentPosY();//拿到当前的Y轴的位置；
        int paddingLeft = getPaddingLeft();//拿到PaddingLeft的值；
        int paddingTop = getPaddingTop();//拿到PaddingTop的值；

        if (mHeaderView != null) {
            MarginLayoutParams lp = (MarginLayoutParams) mHeaderView.getLayoutParams();//拿到Header的layoutParams;
            final int left = paddingLeft + lp.leftMargin;//得到header距离左边距的实际位置；
            final int top = paddingTop + lp.topMargin + offsetX - mHeaderHeight;//padding+margin+curentY-mheaderHeight;//得到当前的View的位置；
            final int right = left + mHeaderView.getMeasuredWidth();//右边的位置；
            final int bottom = top + mHeaderView.getMeasuredHeight();//底部的位置；
            mHeaderView.layout(left, top, right, bottom);//调用layout进行操作，在屏幕上绘制headerView的具体位置；
            if (DEBUG && DEBUG_LAYOUT) {
        ...
        if (mContent != null) {
            ...
            mContent.layout(left, top, right, bottom);//拿到容器的上、下、左、右的位置，进行绘制；
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        if (!isEnabled() || mContent == null || mHeaderView == null) {//当该View不可点、容器为空、头部视图为空的时候，直接走父类的拦截方法；
            return dispatchTouchEventSupper(e);
        }
        int action = e.getAction();//获取用户的动作；
        switch (action) {
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL://当用户动作抬起或者取消的时候，进行以下操作；
                mPtrIndicator.onRelease();//设置mIsUnderTouch为false；
                if (mPtrIndicator.hasLeftStartPosition()) {//判断当前位置；
                    if (DEBUG) {
                        PtrCLog.d(LOG_TAG, "call onRelease when user release");
                    }
                    onRelease(false);
                    if (mPtrIndicator.hasMovedAfterPressedDown()) {
                        sendCancelEvent();//发送取消的Event;
                        return true;
                    }
                    return dispatchTouchEventSupper(e);
                } else {
            ...
            case MotionEvent.ACTION_DOWN://当按压下的时候进行的操作；
                mHasSendCancelEvent = false;
                mPtrIndicator.onPressDown(e.getX(), e.getY());//调用mPtrIndicator中onPressDown的操作；
