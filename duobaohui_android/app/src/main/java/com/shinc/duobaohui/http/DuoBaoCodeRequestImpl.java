 ...
 */
public class DuoBaoCodeRequestImpl implements HttpSendInterFace {
    ...
    public void sendPostRequest(final RequestParams requestParams, String url, final Context context) {
        ...
        MyHttpUtils.getInstance().send(HttpRequest.HttpMethod.POST, url, requestParams, true, false, context, new MyHttpUtils.MyRequestCallBack() {
            DuoBaoCodeBean duoBaoCodeBean = null;

            @Override
            public void onSuccess(ResponseInfo<String> responseInfo) {
                duoBaoCodeBean = GsonUtil.json2Bean(context, responseInfo.result, DuoBaoCodeBean.class);

                EventBus.getDefault().post(new DuoBaoCodeEvent(duoBaoCodeBean));
