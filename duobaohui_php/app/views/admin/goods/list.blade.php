  	...
  	<section class="wrapper">
		<section class="panel">
					...
					<div class='form-group' style="float:right;margin-right:10px;">
						<label>商品分类
							<select class="form-control" style='float:right;width:150px;margin:-5px 0 0 10px;' name="category_id">
								...
								<option value={{ $cv->id }} @if ( $category_id == $cv->id ) selected @endif >{{ $cv->cat_name }}</option>
								@endforeach
							</select>
						</label>
					</div>
			...
			<ul class="pagination pagination-lg">
				@if ( $pageInfo->page > 0 )
				<li><a href="?page={{ $pageInfo->page - 1 }}&goods_name={{ $goods_name }}&category_id={{ $category_id }}">«</a></li>
				@endif
