...
/******************************************登录公用Model*********************************************************************/
		...
		*/
		...
		public function encryptKey($password, $token){
			$userKey = md5($password.KEY_EDCRYPT_SALT.$token);
			return $userKey;
		}
		...
		public function checkSmsCode($tel, $code, $os_type = 1) {
			...
			if ($userData->login_sms_code_expire < time()) return '-2';//验证码失效
				...
				if($userData->is_new_user == 0){
					Log::info("第一次登录时，注册送红包");
					//注册送红包
					$sendRedpacket = new RedpacketService();
					$sendRedpacket->sendNewUserRedpacket($tel);
				}
		 ...
		 * 抹去用户第一次登录的信息
		 */
		public function updateNotNewUser($id){
