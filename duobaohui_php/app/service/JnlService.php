...
class JnlService {
     ...
     * 支付渠道，红包
     */
    const Recharge_Channel_redpacket = '2';
     ...
     * 支付回调，更新流水表信息
     */
    public function payCallbackUpdateJnl($jnl_no,$amount_pay,$jnl_recharge_id,$status,$recharge_channel,$redpacket) {
        ...
        $param = [
            ...
            'update_time' => $this->nowTime,
            'jnl_message' => $msg[$status],
            'repacket_id' => $redpacket
        ];
        $num = $this->jnlTransModel->update($jnl_no,$param);
        return $num > 0 ? true : false;
