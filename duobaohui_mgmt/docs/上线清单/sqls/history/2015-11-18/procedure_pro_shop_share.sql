...
while s <> 1 do
    ...
    insert into sh_shop_share(sh_shop_id,scan_sum,avg_scan_sum,scan_num,real_num,percent,scene_one,scene_two,scene_three,scene_four,insert_date,scene_one_scan,scene_two_scan,scene_three_scan,scene_four_scan,red_packet_percent,days,avg_real_num)
    values(shop_id,scan_count,avg_scan_count,red_count,real_count,ifnull(percent_count,0),scene_one_count,scene_two_count,scene_three_count,scene_four_count,now(),scene_one_scan_count,scene_two_scan_count,scene_three_scan_count,scene_four_scan_count,ifnull(red_packet_percent_count,0),dur_days,avg_real_count);
    #读取下一行的数据
    fetch  cursor_name into shop_id;
