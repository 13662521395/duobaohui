...
ADD COLUMN `scan_sum_android` INT NOT NULL DEFAULT 0 COMMENT 'android扫码数' AFTER `scan_sum`,
ADD COLUMN `scan_sum_ios` INT NOT NULL DEFAULT 0 COMMENT 'ios扫码数' AFTER `scan_sum_android`,
ADD COLUMN `red_packet_android` INT NOT NULL DEFAULT 0 COMMENT 'android 领红包人数' AFTER `scan_num`,
ADD COLUMN `red_packet_ios` INT NOT NULL DEFAULT 0 COMMENT 'ios 领红包人数' AFTER `red_packet_android`;

ALTER TABLE `sh_duobaohui`.`sh_shop_share_history`
ADD COLUMN `scan_sum_android` INT NOT NULL DEFAULT 0 AFTER `scan_sum`,
ADD COLUMN `scan_sum_ios` INT NOT NULL DEFAULT 0 AFTER `scan_sum_android`,
ADD COLUMN `red_packet_android` INT NOT NULL DEFAULT 0 AFTER `red_packet_sum`,
ADD COLUMN `red_packet_ios` INT NOT NULL DEFAULT 0 AFTER `red_packet_android`;
...
while s <> 1 do
    ...
    insert into sh_shop_share(sh_shop_id,scan_sum,scan_sum_android,scan_sum_ios,avg_scan_sum,scan_num,red_packet_android,red_packet_ios,real_num,percent,scene_one,scene_two,scene_three,scene_four,insert_date,scene_one_scan,scene_two_scan,scene_three_scan,scene_four_scan,red_packet_percent,days,avg_real_num)
    values(shop_id,scan_count,scan_android_count,scan_ios_count,avg_scan_count,red_count,red_android_count,red_ios_count,real_count,ifnull(percent_count,0),scene_one_count,scene_two_count,scene_three_count,scene_four_count,now(),scene_one_scan_count,scene_two_scan_count,scene_three_scan_count,scene_four_scan_count,ifnull(red_packet_percent_count,0),dur_days,avg_real_count);
    #读取下一行的数据
    fetch  cursor_name into shop_id;
