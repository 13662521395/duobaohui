...
class ActivityModel extends Model {
	 ...
	 * 抽奖活动列表
	 */
	public function getActivityList($offset, $length, $sort, $strExcludeIds='') {
		/*
		...
		$whereExcludeIds = !empty($strExcludeIds) ? 'and sh_ap.id not in('.$strExcludeIds.')' : '';

		$fields = '`sh_ap`.`id` as `period_id`, `sh_ap`.`current_times`, `sh_ap`.`real_need_times`, `sh_ap`.`create_time`, `sh_a`.`id`, `sh_a`.`max_period`, `sh_a`.`price`, `sh_g`.`id` as `goods_id`, `sh_g`.`goods_name`, `sh_g`.`goods_img`';
		$activityList = DB::select("select {$fields} from `sh_activity_period` as `sh_ap`
						...
						limit ?,?",
					array($this->nowDateTime, $offset, $length));
	...
	public function getActivityDetail($periodId, $loginUserId){
		...
		if($activityDetail->current_times==$activityDetail->real_need_times){
			...
			if($result){
				...
				//状态: 已揭晓
					...
					// 中奖用户
					$rs = DB::select('
						select sum(`times`) as times,`sh_pu`.user_id, `sh_u`.nick_name,`sh_pu`.create_time,`sh_u`.head_pic 
		...
		if($loginUserId){
			...
			if($codeList){
				...
				$activityDetail->login_user->code = $codeList;
	 ...
	 * 即将揭晓列表
	 */
	public function getResultList($offset, $length) {
	...
	public function getPeriodUserList($periodId, $offset, $length, $endId=0){
		if(!$endId){ // 兼容旧版新版上线可删除
			...
			$userList = DB::table( 'period_user AS '.DB::getTablePrefix().'pu' )
				...
				->orderBy('pu.id','DESC')
				->skip($offset)->take($length)
				->select('pu.id','pu.user_id','u.nick_name', 'pu.times', 'pu.create_time', 'u.head_pic')
				->get();
		}else{
			$userList = DB::table( 'period_user AS '.DB::getTablePrefix().'pu' )
				->leftJoin('user AS '.DB::getTablePrefix().'u','u.id','=','pu.user_id')
	...
	public function addPeriodResult($periodId){
		Log::info('---------- 下线');
		// 本期下线
		$isUp = DB::update('UPDATE sh_activity_period SET flag=0 WHERE id=:id AND flag!=0', array('id'=>$periodId));
		if(!$isUp){
			return false;
		}
