...
//        $gmtEndTime   = '2015-11-10 21:17:00';
    	...
    	foreach ($tradeInfo as $key => $value) {
    		$tradeBuyyerInfo[] = array(
    			...
    			'out_trade_no'	=> $value['merchant_out_order_no'],
    			'notify_time'	=> $value['trans_date']
    			);
    	}
	...
	*/
	public function getComparisonInfo( $tradeBuyyerInfo , $tradeNo , $gmtStartTime , $gmtEndTime ){
		...
		$sql = "select out_trade_no,trade_no,total_fee,buyer_id from sh_jnl_alipay where gmt_create between '$gmtStartTime' and '$gmtEndTime'";
		...
		if( !empty( $alipayDataInfo ) ){
			...
			foreach ( $alipayDataInfo as $alipayData ) {
				...
				$hostBuyyerInfo[] = array(
	    			...
	    			'notify_time'	=> $alipayData->notify_time,
    			);
			}
			// 获取去掉重复数据的数组 
				...
				foreach ( $differentTrade as $failAccountTradeNo ) {
					foreach ($tradeBuyyerInfo as $buyyerInfo) {
						if($failAccountTradeNo == $buyyerInfo['trade_no']){
							$isertData[] = array(
								'out_trade_no'	=> $buyyerInfo['out_trade_no'],
								...
								'notify_time'	=> $buyyerInfo['notify_time'],
							);
						}
					}
