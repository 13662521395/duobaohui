...
class script_base{
	...
	function connect() 
	{
		...
		$data_config = include(dirname(__FILE__).'/../../config/database.php');
		//echo 'connecting...'.PHP_EOL.PHP_EOL;;
		$this->mysqlConn= mysql_connect($data_config['connections']['mysql']['host'], $data_config['connections']['mysql']['username'], $data_config['connections']['mysql']['password'], false) or die(mysql_error());
		$sta2 = $this->query("SET NAMES utf8");
		$sta3 = mysql_select_db($data_config['connections']['mysql']['database']) or die(mysql_error());
	}
