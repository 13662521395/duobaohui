...
use \Gate\Package\Helper\DBGoods;
use \Gate\Package\Helper\DBGoodsTest;

use \Gate\Package\Helper\DBLoad;
...
class Demo{
	...
	public function getTableFields($table){
		...
		return DBUserProfile::getConn()->fetchAll($sql, array('database'=>'hayao', 'table_name'=>$table));  //查询方法的第一个参数是sql， 第二个参数是 sql内值
	}

	public function getGoods(){
		$data = array('num'=>1);
		DBLoad::getConn()->insert($data);
		$ids = array();
		for($i=0 ; $i<30;$i++){
			$ids[] = mt_rand(7667593 , 17667593);
		}
		return  DBGoodsTest::getConn()->where('id IN(:ids)' , array('ids'=>$ids))->fetchAll();
	}
